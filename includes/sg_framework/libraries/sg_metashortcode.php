	...
	class SG_MetaShortcode{
		...
		static function init(){
			...
			if(in_array($pagenow, array('post-new.php', 'post.php'))){
				...
				wp_register_script('sg_pool', SG_FRAMEWORK_URL.'/assets/scripts/sg_pool.js', array(), '', false);
		...
		static function _register_button_action($plugin_array){
			$plugin_array['sg_shortcode_gen'] = SG_FRAMEWORK_URL.'/assets/scripts/sg_metashortcode.js';
			foreach(self::$pool as $sc_button){
				$plugin_array[$sc_button->name] = SG_FRAMEWORK_URL.'/assets/scripts/sg_pool.js';
			}
			return $plugin_array;
		}
