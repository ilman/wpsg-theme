...
function get_ea_search_form_new($attr=array()){
	...
	ob_start();	
	include(sg_view_path('templates/ea_api/ea-search-form-'.$layout.'.php'));
	return ob_get_clean();
}
add_shortcode('ea_search_form', 'get_ea_search_form_new');


function get_ea_search_bar_new($attr=array()){
	// extract the attributes into variables
	extract(shortcode_atts(array(
		'action' => '',
		'class' => '',
	...
	ob_start();	
	include(sg_view_path('templates/ea_api/ea-search-bar.php'));
	return ob_get_clean();
}
add_shortcode('ea_search_bar', 'get_ea_search_bar_new');
