...
function sg_get_post_category($post_id=null, $count=null){		
	...
	$output = '';
	for($i=0; $i<$count; $i++){
		$output .= '<span class="post-category"><a href="'.get_category_link($post_cats[$i]->term_id).'">'.$post_cats[$i]->cat_name.'</a></span>';
	}
...
function sg_get_post_author($user_id=false){
	...
	return '<span class="post-author"><a href="'.get_author_posts_url(get_the_author_meta('ID')).'">'.get_the_author_meta('display_name').'</a></span>';
}
